...
import { Plus, Info, Loader2, Inbox } from "lucide-react";
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-gray-500">
            Analyze reference ranges and view clinical alert tags (Hover or click card for clinical tips)
          </p>
        </div>
      </div>
      ...
      <div
        ...
        className={`border rounded-2xl p-5 bg-white shadow-sm transition-all duration-200 ${
          isDragging
            ? "border-[#8989ba] bg-[#8989ba]/10 ring-4 ring-[#8989ba]/20"
            : "border-gray-200"
        }`}
      >
        <div className="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center hover:border-[#8989ba] transition-colors relative cursor-pointer group bg-gray-50/40 hover:bg-[#8989ba]/5">
          ...
          <p className="text-sm font-bold text-gray-800 group-hover:text-[#8989ba] transition-colors">
            {isUploading ? "Uploading & Evaluating Biomarkers..." : "Upload Lab Blood Report or browse"}
          </p>
          <p className="text-xs text-gray-400 mt-1">
            Supports multi-column lab blood report PDFs and images
          </p>
        </div>
      </div>

      {/* Biomarker Cards Grid / Empty State */}
      {displayBiomarkers.length === 0 ? (
        <div className="border border-gray-200 rounded-2xl bg-white p-12 text-center shadow-sm flex flex-col items-center justify-center space-y-2">
          <div className="p-3.5 rounded-full bg-gray-50 text-gray-400 border border-gray-100 mb-1">
            <Inbox size={28} />
          </div>
          <p className="text-sm font-bold text-gray-800">No Blood Biomarkers Evaluated Yet</p>
          <p className="text-xs text-gray-400 max-w-sm">
            Upload a lab blood report PDF or image above to let SmartLab automatically evaluate biomarker status tags and reference ranges.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {displayBiomarkers.map((bio, idx) => {
            const style = getBiomarkerStatusStyle(bio.status);
            const isTipOpen = activeTipId === (bio.id || idx);

            return (
              <div
                key={bio.id || idx}
                onClick={() => setActiveTipId(isTipOpen ? null : (bio.id || idx))}
              ...
              >
                <div>
                  <div className="flex justify-between items-start mb-3 gap-2">
                    ...
                    <span
                      className={`px-2.5 py-0.5 text-xs font-bold rounded-full uppercase tracking-wider flex items-center gap-1.5 shrink-0 ${style.badge}`}
                    >
                      <span
                        className={`h-1.5 w-1.5 rounded-full ${style.markerDot}`}
                      ></span>
                      {bio.status}
                    </span>
                  </div>
                ...
                {/* Floating Gradient Pop-up Tooltip */}
                <div
                  style={{
                    backgroundImage:
